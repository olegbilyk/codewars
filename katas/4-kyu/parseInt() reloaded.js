/*
  ...
  Discuss Link:
    https://www.codewars.com/kata/parseint-reloaded/discuss

  Solutions Link:
    https://www.codewars.com/kata/parseint-reloaded/solutions
...
function parseInt(str) {
  return str.split(/ |-/).reduce((value, word) => {
    ...
    if (numbers[word]) newValue += numbers[word];
    if (discharge[word]) newValue += discharge[word] * (newValue % discharge[word]) - (newValue % discharge[word]);
    return newValue;
  }, 0);
}
...
console.log(parseInt('thirty-seven thousand four hundred twenty-one thirty seven twenty one'), 37421);
